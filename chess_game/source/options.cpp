...
auto example::chess::load_configuration(std::string_view filename) -> Options
{
  std::ifstream fs{std::string{filename}};
  if (!fs.is_open())
    throw apeiron::engine::Error{std::string{"Could not open config file: "} + std::string{filename}};
  ...
  try {
    j = json::parse(ss.str());
    if (j.is_null())
      throw apeiron::engine::Error{std::string{"Config file empty"}};
    ...
    throw apeiron::engine::Error{std::string{"Config file parse error: "} + e.what()};
  }
}


void example::chess::save_configuration(const Options& options, std::string_view filename)
{
  json j;
