...
void example::chess::Menu::setup(Options* options)
{
...
void example::chess::Menu::build(Options* options, float time)
{
  auto& io = ImGui::GetIO();
  ...
  create_frame();
  {
    ...
    ImGui::Checkbox("Axes", &options->show_axes);
    ImGui::SameLine();
    ImGui::Checkbox("Ground", &options->show_ground);
    ImGui::Text("Camera");
